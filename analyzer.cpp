...
static inline bool TripAnalyzer::is_space_(unsigned char c) {
...
bool TripAnalyzer::parseHourFromTimestamp_(const std::string& raw, int& outHour) {
    ...
    while (i >= 0 && is_space_(static_cast<unsigned char>(s[i]))) --i;
    if (i < 0 || !std::isdigit(static_cast<unsigned char>(s[i]))) return false;
    ...
    int tens = -1;
    if (i >= 0 && std::isdigit(static_cast<unsigned char>(s[i]))) {
        tens = s[i] - '0';
    }
...
bool TripAnalyzer::looksLikeHeader_(const std::vector<std::string>& cols) {
    ...
    if (lower(cols[0]).find("trip") != std::string::npos) return true;
    if (cols.size() >= 2 && lower(cols[1]).find("zone") != std::string::npos) return true;
    if (cols.size() >= 3 && lower(cols[2]).find("time") != std::string::npos) return true;
...
std::vector<SlotCount> TripAnalyzer::topBusySlots(int k) const {
    ...
    for (const auto& p : zoneHourTotals_) {
        for (int h = 0; h < 24; ++h) {
            ...
            if (c > 0) v.push_back({p.first, h, c});
        }
    }
